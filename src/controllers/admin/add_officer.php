...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO Officer 
        (Officer_uname, Officer_pwd, First_name, Last_name, Title, Date_of_birth)
        VALUES (?, ?, ?, ?, ?, ?)
    ");

    $stmt->execute([$uname, $pwd, $fname, $lname, $title, $dob]);

    redirect('add_officer&success=1');
    exit;
}
?>
...
<div class="flex flex-col text-center mb-10">
    <h1 class="text-3xl font-bold mb-2">Add Officer</h1>
    <p class="text-gray-600">Register a new officer</p>
</div>
...
<div class="p-2 w-1/2">
<label>Title</label>
<input name="title" required class="w-full bg-gray-100 border px-3 h-10 rounded">
</div>
