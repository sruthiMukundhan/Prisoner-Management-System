<?php
    if(isset($_POST['jailor_add'])){
      ...
      if(empty($f_name)||empty($l_name)||empty($mob_number)||empty($username)||empty($password)||empty($cfmpassword)){   
        header("Location: ../../public/?page=jailor&error=emptyfields");
        exit();
    } else if(!preg_match("/^[a-zA-Z0-9]*$/", $username)){
      header("Location: ../../public/?page=jailor&error=invaliduid&uid=".$username);
       exit();
   } else if($password !== $cfmpassword){
    header("Location: ../../public/?page=jailor&error=passwordnotmatched&uid=".$username);
    exit();
  } else{
    $sql="SELECT Jailor_uname FROM Jailor WHERE Jailor_uname=?";
    $stmt=mysqli_stmt_init($conn);
      if(!mysqli_stmt_prepare($stmt,$sql)){
        header("Location: ../../public/?page=jailor&error=sqlerror");
        exit();
        } else{
          mysqli_stmt_bind_param($stmt,"s",$username);
                ...
                if($resultCheck>0){
                    header("Location: ../../public/?page=jailor&error=sameuserexistserror");
                    exit();
                }
                else{
                  ...
                  if(!mysqli_query($conn,$sql_disable1)){
                    header("Location: ../../public/?page=jailor&error=sqlerror1");
                    exit();
                  }
                  $sql_disable2="ALTER TABLE Jailor_phone DISABLE KEYS ";
                  if(!mysqli_query($conn,$sql_disable2)){
                    header("Location: ../../public/?page=jailor&error=sqlerror11");
                    exit();
                  }
                  /* 
                  ...
                  */
                  ...
                  if(!mysqli_query($conn,$sql0)){
                    header("Location: ../../public/?page=jailor&error=sqlerror2");
                    exit();
                  }
                  ...
                  $sql1="INSERT INTO Jailor (Jailor_uname, Jailor_pwd, First_name, Last_name) VALUES (?,?,?,?) ";
                          ...
                          if(!mysqli_stmt_prepare($stmt1,$sql1)){
                              header("Location: ../../public/?page=jailor&error=sqlerror3");
                              exit();
                          }else{
                              //hashing the password:
                  ...
                  if(!mysqli_stmt_prepare($stmt2,$sql2)){
                          header("Location: ../../public/?page=jailor&error=sqlerror4");
                            exit();
                  }else{
                  ...
                  if(!mysqli_stmt_prepare($stmt3,$sql3)){
                     header("Location: ../../public/?page=jailor&error=sqlerror5");
                      exit();
                  }else{
                  ...
                  if(!mysqli_query($conn,$sql_enable1)){
                    header("Location: ../../public/?page=jailor&error=sqlerror6");
                    exit();
                  }else{
                    ...
                    header("Location: ../../public/?page=successjailor&insert=success");
                    exit();
                  }
                }
        ...
        header("Location: ../../public/?page=jailor&error=clickonsignupbtnerror");
        exit();
